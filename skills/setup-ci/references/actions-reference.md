...
- **特有输入**: `default-team`（定义 reviewer 组合，默认 `quality:1,security:1,performance:1,architecture:1,regression-test:1,test-value:1`）、`coordinator-timeout-seconds`、`coordinator-prompt`、`gitea-token`、`diff-exclude`、`diff-max-size-kb`、`language`、`reasoning-effort`、`enable-thinking`

### regression-test-missing — 回归测试缺失检测
- **用途**: 检测 bug 修复或行为变更的 PR 是否缺少对应的回归测试
- **触发**: `pull_request` (opened, synchronize, reopened, ready_for_review)
- **输出语言**: 中文（默认），首行给出判定（无需回归测试 / 缺少回归测试）
- **特有输入**: `gitea-token`、`language`

### test-value-detector — 低价值测试检测
...
| `default-team` | `quality:1,security:1,performance:1,architecture:1,regression-test:1,test-value:1` | reviewer 组合（如 `"quality:2,security:1"`） |
...
| `reasoning-effort` | `max` | 推理强度：`low` / `medium` / `high` / `max` |
| `enable-thinking` | `true` | 启用 thinking 模式 |
| `diff-exclude` | empty | 排除的 glob 模式（逗号分隔），锁定文件始终排除 |
| `diff-max-size-kb` | `0` | 过滤后 diff 最大 KB，超出截断（`0` 禁用） |
| `github-token` | empty | GitHub token |
...
# regression-test-missing: contents: read, pull-requests: write, issues: write
# test-value-detector:     contents: read, pull-requests: write
...
multi-review 会同时启动多个 reviewer + 1 个 coordinator 的 SDK 会话（通常 6–7 个并发请求）。以下 provider 的订阅计划存在并发限制，**不兼容 multi-review**：
...
> 这些 provider 的单 agent action（`review`、`architect-review`、`feature-missing`、`regression-test-missing`、`test-value-detector` 等）不受影响。
