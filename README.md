...
## Combined PR Checks Workflow

For convenience, a combined workflow file (`.github/workflows/pr-checks.yml`) is provided that runs all three checks (`review`, `feature-missing`, `regression-test-missing`) as parallel jobs in a single workflow. Copy it to your repository to get all three checks with one workflow file.
...
- smoke-tests all actions through `uses: ./setup-opencode`, `uses: ./run-opencode`, `uses: ./github-run-opencode`, `uses: ./review`, `uses: ./feature-missing`, and `uses: ./regression-test-missing`
...
5. Use `owner/repo/review@v2` for the simplest review setup, `owner/repo/feature-missing@v2` for spec coverage audit, `owner/repo/regression-test-missing@v2` for regression test audit, `owner/repo/github-run-opencode@v2` for generic `github run`, or `owner/repo/setup-opencode@v2` plus `owner/repo/run-opencode@v2` for more control.
